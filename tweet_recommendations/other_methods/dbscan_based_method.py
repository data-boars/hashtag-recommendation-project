...
# "Finally, we use one as the number of minPoints and the average value of
# this vector as epsilon." # according to publication, this value should be
# fixed to 1.
MIN_SAMPLES = 1
...
class DBScanBasedMethod(Method):
    def __init__(
        ...
        """
        Hashtag recommendation method implementation based on
        https://bit.ly/2V3tnWX.
        ...
        self.sif_embedding = SIFEmbedding(
            path_to_keyedvectors_model, verbose=verbose
        )
    ...
    def fit(
        ...
        """
        Builds hashtags representations using w2v method and clusters them
        using DBScan.
        ...
        if self.verbose:
            print(
                f"Data embedding shape, after droping data by given criteria:"
                f" {embeddings.shape}"
            )
        ...
        x["hashtags"] = x["hashtags"].apply(
            lambda r: [elem["text"] for elem in r]
        )
        ...
        if self.verbose:
            print(
                f"Samples grouped into "
                f"{len(np.unique(self.clusterizer.labels_))} clusters"
            )
        ...
        for cluster_number, cluster in tqdm.tqdm(
            grouped_clusters,
            total=len(np.unique(x["cluster_label"])),
            ...
            distances = distance.cdist(
                cluster_center, cluster_embeddings, metric="cityblock"
            )[0]

            cluster_centroid_index = np.argmin(distances)
            # we are sure that it will not point to itself because mean
            # center should be not existent
            ...
            sorted_most_popular_hashtags = [
                hashtag
                for hashtag, count in Counter(
                    aggregated_hashtags
                ).most_common()
            ]

            self._centroids_data.append(cluster_centroid)
            self._corresponding_to_centroids_data_hashtags.append(
                sorted_most_popular_hashtags
            )

        self._centroids_data = np.vstack(self._centroids_data)
        self._corresponding_to_centroids_data_hashtags = np.asarray(
            self._corresponding_to_centroids_data_hashtags
        )
    ...
    def transform(
        ...
        :param x: tuple of tuple of str or tuple of str. If first argument of
            x is a tuple is str, it is assumed that tuple contains already
            lemmatized text. If single str is present as first element, it is
    ...
    def post_process_result(
        ...
        is leftover from the `fit` because tags inside a single cluster are
        sorted according to their popularity.

        :param recommended_hashtags: Two level list of recommended hashtags
            for each tweet and each tweet has clusters of tags.
