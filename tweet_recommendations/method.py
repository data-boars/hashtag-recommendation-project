...
class Method(TransformerMixin, BaseEstimator):
    ...
    @classmethod
    def drop_tweets_with_hashtags_that_occurred_less_than(
        cls, data: pd.DataFrame, minimal_hashtag_occurrence: int
    ) -> pd.DataFrame:
        ...
        filtered_tags = {
            ...
            if count >= minimal_hashtag_occurrence
        }
        ...
        data = data.drop(
            data[data["hashtags"].str.len() == 0].index
        ).reset_index()
        return data
